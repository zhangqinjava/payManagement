...
import com.al.mapper.MerchantMapper;
...
public class MerchantServiceImpl implements MerchantService {
    ...
    public MerchantVo save(MerchantDto merchantDto) throws Exception {
        try {
            ...
            MerchantVo build = MerchantVo.builder()
                    .merchantNo(TraceUtil.createTraceId())
            ...
            if(e instanceof DuplicateKeyException){
                ...
                MerchantVo build = MerchantVo.builder()
                        .merchantNo(merchantDto.getMerchantNo())
                        .merchantName(merchantDto.getMerchantName())
                        .merchantType(merchantDto.getMerchantType())
                        .status(MerchantEnum.NOMAL.getCode())
                        .contactName(merchantDto.getContactName())
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public String delete(String merchantNo) throws Exception{
        try {
    ...
    @Override
    public MerchantVo query(String merchantNo) throws Exception{
        try {
            log.info("merchant information query start:{} ", merchantNo);
            if (StringUtils.isEmpty(merchantNo)) {
                throw new BusinessException("商户号不能为空!");
            }
            // 1. 先查缓存
            MerchantVo merchant = genericCache.get(Const.MERCHANT_PREFIX + merchantNo, MerchantVo.class);
            if (merchant != null) {
                return merchant;
            }
            // 2. 空值缓存判断
            if (genericCache.isNullCached(Const.MERCHANT_PREFIX + merchantNo)) {
                return null;
            }
            RLock lock = redissonClient.getLock(Const.MERCHANT_LOCK + merchantNo);
            try {
                // 3. 防击穿加锁
                if (lock.tryLock(100, TimeUnit.MILLISECONDS)) {
                    // double check
                    merchant = genericCache.get(Const.MERCHANT_PREFIX + merchantNo, MerchantVo.class);
                    if (merchant != null) {
                        return merchant;
                    }
                    // 4. DB 回源
                    merchant = merchantMapper.selectOne(Wrappers.lambdaQuery(MerchantVo.class).eq(MerchantVo::getMerchantNo, merchantNo));
                    if (merchant == null) {
                        genericCache.cacheNull(Const.MERCHANT_NULL_PREFIX, merchantNo);
                        return null;
                    }
                    // 5. 写缓存
                    genericCache.set(Const.MERCHANT_PREFIX + merchant.getMerchantNo(), merchant);
                    return merchant;
                } else {
                    // 没拿到锁，短暂 sleep 后再读缓存
                    Thread.sleep(20);
                    return genericCache.get(Const.MERCHANT_PREFIX + merchantNo, MerchantVo.class);
                }
            } catch (Exception e) {
